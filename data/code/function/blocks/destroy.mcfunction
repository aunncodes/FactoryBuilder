...
data modify entity @n[type=item,tag=item.target,distance=..3] Thrower set from storage temp uuid.uuid
...
visibility @a hide @n[type=item,tag=item.target,distance=..3]
execute as @n[type=item,tag=item.target,distance=..3] on origin run visibility @s show @n[type=item,tag=item.target,distance=..3]
